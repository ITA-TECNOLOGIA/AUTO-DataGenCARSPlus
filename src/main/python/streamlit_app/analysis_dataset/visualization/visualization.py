...
from streamlit_app.analysis_dataset.visualization import visualization_rating, visualization_uicb
...
def generation(with_context):
    ...
    if user_tab:
        with user_tab:
            ...
            visualization_uicb.show_information(df=user_df, file_type=config.USER_TYPE)
    # TAB --> Items:
    if item_tab:
        with item_tab:
            # Loading item file:
            ...
            visualization_uicb.show_information(df=item_df, file_type=config.ITEM_TYPE)
    # TAB --> Contexts:
    if context_tab:
        with context_tab:
            ...
            visualization_uicb.show_information(df=context_df, file_type=config.CONTEXT_TYPE)
    # TAB --> Behaviors:
    if behavior_tab:
        with behavior_tab:
            ...
            visualization_uicb.show_information(df=behavior_df, file_type=config.BEHAVIOR_TYPE)
    # TAB --> Ratings:
    if rating_tab:
        with rating_tab:
            # Loading rating file:
            ...
            # Visualizing rating information:
            visualization_rating.show_information(rating_df=rating_df, with_context=with_context)   
