...
def build_knowledge_base(rating_df):
    ...
    if (not item_df.empty) and (not context_df.empty):                
        with st.sidebar.expander(label='**Contextual features selection**'):
        ...
        if 'context_id' not in rating_df.columns:
            ...
            if 'timestamp' in rating_df.columns:
                knowledge_base_df.drop('timestamp', axis=1, inplace=True)
...
def select_contextual_features(df, label):
    ...
    selected_columns = st.multiselect(label=f'Select {label} features:', options=column_names, default=column_names)
    if selected_columns:
...
def select_classification_algorithm():
    ...
    with st.sidebar.expander(label='**Classification algoritms selection**'):    
        classifier_name_list = st.multiselect("Select one or more classifiers", config.CLASSIFIER_OPTIONS, default=config.CLASSIFIER_OPTIONS[0])
        st.write("-0.5 values will be replaced with None") # why?
    ...
    for classifier_name in classifier_name_list:
        with st.sidebar.expander(label=f'**{classifier_name} settings**'):
...
def select_classification_parameters_map(classification_algorithm):
    ...
    if classification_algorithm == "KNeighborsClassifier":
        return {"n_neighbors": st.number_input("Number of neighbors", min_value=1, max_value=1000, value=5, key='n_neighbors_kneighborsclassifier'),
                "weights": st.selectbox("Weights", ["uniform", "distance"], key='weights_kneighborsclassifier'),
                "algorithm": st.selectbox("Algorithm", ["auto", "ball_tree", "kd_tree", "brute"], key='algorithm_kneighborsclassifier'),
                "leaf_size": st.number_input("Leaf size", min_value=1, max_value=1000, value=30, key='leaf_size_kneighborsclassifier'),
                "p": st.number_input("P", min_value=1, max_value=1000, value=2, key='p_kneighborsclassifier'),
                "metric": st.selectbox("Metric", ["minkowski", "euclidean", "manhattan", "chebyshev", "seuclidean", "mahalanobis", "wminkowski", "haversine"], key='metric_kneighborsclassifier'),
                "n_jobs": st.number_input("Number of jobs", min_value=1, max_value=1000, value=1, key='n_jobs_kneighborsclassifier')}
    elif classification_algorithm == "SVC":
        return {"C": st.number_input("C", min_value=0.0001, max_value=1000.0, value=1.0, key='C_svc'),
                "kernel": st.selectbox("Kernel", ["linear", "poly", "rbf", "sigmoid", "precomputed"], key='kernel_svc'),
                "degree": st.number_input("Degree", min_value=1, max_value=1000, value=3, key='degree_svc'),
                "gamma": st.selectbox("Gamma", ["scale", "auto"], key='gamma_svc'),
                "coef0": st.number_input("Coef0", min_value=0.0, max_value=1000.0, value=0.0, key='coef0_svc'),
                "shrinking": st.checkbox("Shrinking?", key='shrinking_svc'),
                "probability": st.checkbox("Probability?", key='probability_svc'),
                "tol": st.number_input("Tol", min_value=0.0001, max_value=1000.0, value=0.001, key='tol_svc'),
                "cache_size": st.number_input("Cache size", min_value=0.0001, max_value=1000.0000, value=200.0000, key='cache_size_svc'),
                "class_weight": st.selectbox("Class weight", ["balanced", None], key='class_weight_svc'),
                "verbose": st.checkbox("Verbose?", key='verbose_svc'),
                "max_iter": st.number_input("Maximum iterations", min_value=-1, max_value=1000, value=-1, key='max_iter_svc'),
                "decision_function_shape": st.selectbox("Decision function shape", ["ovo", "ovr"], key='decision_function_shape_svc'),
                "random_state": st.number_input("Random state", min_value=-0.5, max_value=1000.0, value=-0.5, key='random_state_svc'),
                "break_ties": st.checkbox("Break ties?", key='break_ties_svc')}
    elif classification_algorithm == "GaussianNB":
        return {}
    elif classification_algorithm == "RandomForestClassifier":
        return {"n_estimators": st.number_input("Number of estimators", min_value=1, max_value=1000, value=100, key='n_estimators_randomforestclassifier'),
                "criterion": st.selectbox("Criterion", ["gini", "entropy"], key='criterion_randomforestclassifier'),
                "max_depth": st.number_input("Maximum depth", min_value=-0.5, max_value=1000.0, value=-0.5, key='max_depth_randomforestclassifier'),
                "min_samples_split": st.number_input("Minimum samples split", min_value=1, max_value=1000, value=2, key='min_samples_split_randomforestclassifier'),
                "min_samples_leaf": st.number_input("Minimum samples leaf", min_value=1, max_value=1000, value=1, key='min_samples_leaf_randomforestclassifier'),
                "min_weight_fraction_leaf": st.number_input("Minimum weight fraction leaf", min_value=0.0001, max_value=1.0, value=0.01, key='min_weight_fraction_leaf_randomforestclassifier'),
                "max_features": st.selectbox("Maximum features", ["auto", "sqrt", "log2", None], key='max_features_randomforestclassifier'),
                "max_leaf_nodes": st.number_input("Maximum leaf nodes", min_value=-0.5, max_value=1000.0, value=-0.5, key='max_leaf_nodes_randomforestclassifier'),
                "min_impurity_decrease": st.number_input("Minimum impurity decrease", min_value=0.0001, max_value=1.0, value=0.01, key='min_impurity_decrease_randomforestclassifier'),
                "bootstrap": st.checkbox("Bootstrap?", key='bootstrap_randomforestclassifier'),
                "oob_score": st.checkbox("OOB score?", key='oob_score_randomforestclassifier'),
                "n_jobs": st.number_input("Number of jobs", min_value=-0.5, max_value=1000.0, value=-0.5, key='n_jobs_randomforestclassifier'),
                "random_state": st.number_input("Random state", min_value=-0.5, max_value=1000.0, value=-0.5, key='random_state_randomforestclassifier'),
                "verbose": st.number_input("Verbose", min_value=0, max_value=1000, value=0, key='verbose_randomforestclassifier'),
                "ccp_alpha": st.number_input("CCP alpha", min_value=0.0001, max_value=1.0, value=0.01, key='ccp_alpha_randomforestclassifier'),
                "class_weight": st.selectbox("Class weight", ["balanced", "balanced_subsample", None], key='class_weight_randomforestclassifier'),
                "max_samples": st.number_input("Maximum samples", min_value=-0.5, max_value=1.0, value=-0.5, key='max_samples_randomforestclassifier'),
                "warm_start": st.checkbox("Warm start?", key='warm_start_randomforestclassifier')}
    elif classification_algorithm == "KMeans":
        return {"n_clusters": st.number_input("Number of clusters", min_value=1, max_value=1000, value=5, key='n_clusters_kmeans'),
                "init": st.selectbox("Initialization method", ["k-means++", "random"], key='init_kmeans'),
                "n_init": st.number_input("Number of initializations", min_value=1, max_value=1000, value=10, key='n_init_kmeans'),
                "max_iter": st.number_input("Maximum number of iterations", min_value=1, max_value=1000, value=300, key='max_iter_kmeans'),
                "tol": st.number_input("Tolerance", min_value=0.0001, max_value=1.0, value=0.0001, key='tol_kmeans')}
    elif classification_algorithm == "HistGradientBoostingClassifier":
        return {"learning_rate": st.slider("Learning rate", 0.01, 1.0, 0.1, step=0.01),
                "max_iter": st.slider("Max number of iterations", 10, 1000, 100, step=10),
                "max_leaf_nodes": st.slider("Max leaf nodes", 10, 200, 31, step=1),
                "max_depth": st.slider("Max depth", 1, 50, 15, step=1),
                "l2_regularization": st.slider("L2 regularization", 0.0, 1.0, 0.0, step=0.01)}
...
def select_split_strategy():
    ...
    :return: A dictionary with parameter values. 
    """
    with st.sidebar.expander(label='**Split strategy selection**'):       
        split_strategy = st.selectbox("Select a strategy", ["KFold", "ShuffleSplit", "LeaveOneOut"]) # , "PredefinedKFold", "train_test_split", "RepeatedKFold",
        split_strategy_parameter_dict = {}
        if split_strategy == "ShuffleSplit":
            n_splits = st.number_input("Number of splits", 2, 100, 10)
            train_size = st.slider("Train set size (0.0 to 1.0)", 0.01, 1.0, 0.2, step=0.01)
            split_strategy_parameter_dict = {"n_splits": n_splits, "train_size": train_size}
        elif split_strategy == "KFold":
            n_splits = st.number_input("Number of folds", 2, 100, 5)
            split_strategy_parameter_dict = {"n_splits": n_splits, "shuffle": st.checkbox("Shuffle?")}
...
def select_evaluation_metric():  
    ...
    """
    with st.sidebar.expander(label='**Metrics selection**'):          
        metric_list = st.multiselect("Select one or more metrics", config.SCIKIT_LEARN_METRICS, default=["Precision", "Recall", "F1 score"])
...
def evaluate(knowledge_base_df, classifier_list, split_strategy_instance, metric_list):
    ...
    if not knowledge_base_df.empty:        
        with st.sidebar.expander(label='**Evaluation users selection**'):
            ...
            selected_users = st.multiselect("Select one or more users or 'All users'", options=user_options, default="All users")
        ...
        st.warning('The knowledge base is empty. Are contextual features defined?')
...
def show_evaluation_result(evaluation_result_df, selected_users):
    ...
    if "evaluation_result_df" in st.session_state:
        ...
        global_mean_results_df = evaluation_result_df.groupby('Algorithm').mean()
        # Remove the 'Fold' and 'User' columns:
        global_mean_results_df = global_mean_results_df.drop(['Fold', 'User'], axis=1)

        # Showing result dataframe by mean:
        st.dataframe(global_mean_results_df)    
        # Save dataframe:                
        wf_util.save_df(df_name='mean_evaluation_results', df_value=global_mean_results_df, extension='csv') 
