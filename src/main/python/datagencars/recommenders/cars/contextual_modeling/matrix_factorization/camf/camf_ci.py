...
class CAMF_CI(CAMF):
    """
    Context-aware Matrix Factorization with Item-Condition Deviations (CAMF_CI).
    
    This model introduces one parameter per contextual condition and item pair. 
    For instance, sunny weather could increase the rating of the flea market but not the museum of natural science.
    
    Original Version: Java (https://github.com/irecsys/CARSKit/)
        CAMF_CI: Baltrunas, Linas, Bernd Ludwig, and Francesco Ricci. "Matrix factorization techniques for context-aware recommendation."
        Note: This algorithm introduces a rating deviation for each item-condition pair.
        @author Yong Zheng
    """
    ...
    def __init__(self, train_matrix, test_matrix, context_train, context_test, num_conditions=5, num_factors=10, num_interactions=100, init_learning_rate=0.01, reg=0.02, reg_b=0.1, reg_u=0.1, reg_i=0.1, reg_c=0.1):
        ...
        Args:
            ...
            reg_c (float): Regularization for item-condition biases.
        """
    ...
    def predict(self, user, item, context_list):
        ...
        # Add item-condition biases
        for idx, cond in enumerate(context_list):
            prediction += self.item_condition_bias[item, idx]

        return prediction

    def build_model(self):
        """
        Train the CAMF_CI model using Stochastic Gradient Descent (SGD).
        """
        for iteration in range(1, self.num_interactions + 1):
            ...
            for idx, row in self.train_df.iterrows():
                ...
                # Update item-condition biases
                for idx, cond in enumerate(context_list):
                    bias = self.item_condition_bias[item, idx]
                    self.item_condition_bias[item-1, idx] += self.learning_rate * (error - self.reg_c * bias)
    ...
    def __str__(self):
        ...
        """
        return f"{super().__str__()}, CAMF_CI with item-condition deviations"
...
#     model = CAMF_CI(train_data, test_data, context_train, context_test, num_conditions=5, num_factors=10, num_interactions=100, init_learning_rate=0.01, reg=0.02, reg_b=0.1, reg_u=0.1, reg_i=0.1, reg_c=0.1)
