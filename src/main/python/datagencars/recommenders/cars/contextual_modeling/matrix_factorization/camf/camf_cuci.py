...
class CAMF_CUCI(CAMF):
    """
    Context-aware Matrix Factorization with User-Condition and Item-Condition Deviations (CAMF_CUCI).

    This model introduces rating deviations for both user-condition and item-condition pairs, capturing the joint effect
    of user-specific preferences and item-specific preferences under various contextual conditions.
    ...
    def __init__(self, train_matrix, test_matrix, context_train, context_test, num_conditions=5, num_factors=10,
                 num_interactions=100, init_learning_rate=0.01, reg=0.02, reg_b=0.1, reg_u=0.1, reg_i=0.1, reg_c=0.1):
        """
        Initialize the CAMF_CUCI model.
        ...
        Args:
            ...
            reg_c (float): Regularization for condition biases.
        """
        super().__init__(train_matrix, test_matrix, context_train, context_test, num_conditions, num_factors,
                         num_interactions, init_learning_rate, reg, reg_b, reg_u, reg_i, reg_c)
        ...
        self.item_condition_bias = np.random.normal(0, 0.1, (self.num_items, self.num_conditions))
    ...
    def predict(self, user, item, context_list):
        ...
        # Add user-condition and item-condition biases
        for idx, cond in enumerate(context_list):
            prediction += (
                self.user_condition_bias[user, idx] +
                self.item_condition_bias[item, idx]
            )
    ...
    def build_model(self):
        """
        Train the CAMF_CUCI model using Stochastic Gradient Descent (SGD).
        """
        for iteration in range(1, self.num_interactions + 1):
            ...
            for _, row in self.train_df.iterrows():
                ...
                # Update user-condition and item-condition biases
                for idx, cond in enumerate(context_list):
                    self.user_condition_bias[user, idx] += self.learning_rate * (
                        error - self.reg_c * self.user_condition_bias[user, idx]
                    )
                    self.item_condition_bias[item, idx] += self.learning_rate * (
                        error - self.reg_c * self.item_condition_bias[item, idx]
                    )
    ...
    def __str__(self):
        ...
        """
        return f"{super().__str__()}, CAMF_CUCI with user-condition and item-condition deviations"
...
#     model = CAMF_CUCI(train_data, test_data, context_train, context_test, num_conditions=5, num_factors=10, num_interactions=100, init_learning_rate=0.01, reg=0.02, reg_b=0.1, reg_u=0.1, reg_i=0.1, reg_c=0.1)
